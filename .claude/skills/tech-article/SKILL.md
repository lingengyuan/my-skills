...
**有大量量化数据时（评测/实验/性能报告）：**
- 找出**最戏剧性的一对对比**——不是平均分，而是"同一个参数改一行，分数从 3.27 变成 8.39"这种
- 确定**叙事脊梁**：整篇文章只需要一个核心故事线（如"一个 bug 如何改变排名"），其他数据围绕它展开，不要把所有维度摊平成并列关系
- 量化数据必须转化为人能感知的单位——"生成一道代码题需要 48 分钟"比"12,784 个 thinking token"更有冲击力；先写人的感受，再带出原始数字
- 数据图表：matplotlib 生成的图优先用于文章，不要只引用原始数字表格
...
- 过度归纳：反直觉（偶尔可用，但一篇文章最多出现 1 次；不能当万能标签）、核心发现、关键洞察、最关键的发现、最重要的是
- AI 过渡句："这让我想到一个更普遍的规律"、"这说明了一个重要的事实"、"让我们来看看"、"接下来我们将探讨"
- 煽情定性句："这是整个[评测/过程/项目]里最[X]的[Y]"——这类句式用来放大戏剧性，实际上是作者替读者下判断，删掉让数据自己说话
- 道德总结句："这是一个值得记住的教训"、"这值得所有做[X]的人注意"——事实讲完就讲完，不需要告诉读者这件事重不重要

**禁用结构模式：**
- "发现一/发现二/发现三"式机械编号——用自然的小标题代替，每个标题说具体的事而不是编号
- 开篇的论文式摘要——不要写"这篇文章讲三件事：第一……第二……第三……"；也不要写"这篇文章写的不只是结论，而是整个过程"这类"预告片"句，让读者自己读到
- 每节末尾的"总结升华"——节的最后一句话应该是具体的事实或观点，不是对本节的复述
- 前后呼应的排比句——"不是因为 A，而是因为 B；不是因为 C，而是因为 D"用一次可以，用两次就有味道了
- 同一层级下多个等权重 H3——5 个问题不需要 5 个等重的 H3 小节；小问题合并进散文段落，只有真正独立且值得展开的才单独成节
- 结论用 bullet 列举——3-4 条 bullet 的结论段是最典型的 AI 腔，改成 1-2 段散文，用因果或转折把要点串起来
...
| Type | Target | 适用场景 |
|------|--------|----------|
| Short post | 800–1200字 | 单一技巧、快速分享 |
| Standard | 1200–2000字 | 工具介绍、一般洞察 |
| Deep dive | 2000–3500字 | 多维比较、系统研究 |
| 深度报告 | 4000–8000字 | 有大量原始数据（评测/实验）、踩坑记录详细、需要图表支撑；篇幅由数据密度决定，不要为凑字数而拉长 |

**深度报告的额外要求：**
- 每个主要章节要有实际数据或原始代码支撑，不能只是叙述
- 章节节奏要有起伏——不是每节都同样长，重要发现多写，周边细节少写
- 图表（matplotlib/数据可视化）是标配，而非可选
...
- [ ] 结尾是一个短问题或一个具体事实，不是对全文的复述
- [ ] 技术细节有原始数据或代码支撑（不是泛泛描述）
- [ ] 没有以"希望本文对大家有所帮助"结尾
- [ ] **去 AI 味检查**：全文搜索禁用词表中的词，逐个替换或删除；检查是否有"发现一/二/三"式编号结构
- [ ] **煽情定性句检查**：搜索"这是整个X里最Y的Z"、"这是一个值得记住的教训"等句式，一律删除
- [ ] **结构检查**：同一节内是否有 3 个以上等权重 H3？如果有，考虑将较小的问题合并成散文段落
- [ ] **结论段检查**：结论是否是 bullet list？如果是，改成散文段落
